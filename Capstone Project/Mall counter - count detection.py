...
lcd_init()
lcd_string("MALL COUNTER",LCD_LINE_1)
time.sleep(1)
...
while 1:
  ...
  if GPIO.input(IR_Sensor):
   if count<25:
    ...
    lcd_string("Count : "+str(count) ,LCD_LINE_2)
    GPIO.output(buzzer,True)
    time.sleep(.2)
    GPIO.output(buzzer,False)
   else:
    lcd_string("No Entry",LCD_LINE_1)
    lcd_string("Maxi Limit:25",LCD_LINE_2)
  else:
    lcd_string("no person",LCD_LINE_1) 
  time.sleep(1)
